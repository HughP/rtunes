...
var CirclePlayer = function(jPlayerSelector, media, options) {
	var	self = this,
		...
		cssSelector = {
			volumeControl: ".cp-volume-control",
			volume1: ".cp-volume-1",
			volume2: ".cp-volume-2",
	...
	this.timeUpdate = (options.timeUpdate) ? options.timeUpdate : function(){};
	this.seekTimeUpdate = (options.seekTimeUpdate) ? options.seekTimeUpdate : function(){};
...
CirclePlayer.prototype = {
	...
	_initPlayer: function() {
		...
		this.player.bind($.jPlayer.event.timeupdate + this.eventNamespace, function(event) {
			if (!self.dragging) {
				self.timeUpdate(event.jPlayer.status.currentTime, self.media);
	...
	_initCircleControl: function() {
		...
		this.jq.circleControl.grab({
			onstart: function(){
				...
				self._timeupdate(pc);
				self.seekTimeUpdate(pc, self.media);
